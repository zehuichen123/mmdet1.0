...
class FutureDataset(CustomDataset):
    ...
    CLASSES = [str(ii) for ii in range(34)]
    ...
    def segm2json_segm(self, results):
        segm_json_results = []
        for idx in range(len(self)):
            img_id = self.img_ids[idx]
            seg = results[idx]
    ...
    def results2json_segm(self, results, out_file):
        result_files = dict()
        json_results = self.segm2json_segm(results)
    ...
    def format_results(self, results, jsonfile_prefix=None, **kwargs):
        ...
        result_files = self.results2json_segm(results, jsonfile_prefix)
